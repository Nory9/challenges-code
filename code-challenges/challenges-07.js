...
const objLat = (obj) => {
    const first =obj.firstName[0].toUpperCase() + obj.firstName.slice(1);
    const last =obj.lastName[0].toUpperCase() + obj.lastName.slice(1);
...
const cvFormatter = (arr) => {
    ...
    if(arr[i].yearsOfExperience>1){
        ...
        if(arr[i].lastName!==null){
            name += (name.length > 0 ? ' ' : '') + arr[i].lastName;
            // name+=arr[i].lastName;
        }
        outputArr.push({
            fullName: name,
